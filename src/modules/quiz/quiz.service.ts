...
export async function submitAnswer(sessionId: string, questionId: string, answer: string) {
  ...
  const deadline = session.startedAt.getTime() + ((questionIndex + 1) * session.timePerQuestionSeconds * 1000); const expired = Date.now() > deadline; const question = session.questions[questionIndex];
  ...
  session.answers.push({ questionId, answer, status, pointsAwarded, answeredAt }); session.score += pointsAwarded; await session.save();
