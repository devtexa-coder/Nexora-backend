...
io.on("connection", (socket) => {
  socket.on("room:join", async ({ roomId, playerId }: { roomId: string; playerId: string }) => { const room = await SocialRoom.findOne({ id: roomId, "members.playerId": playerId }); if (!room) return socket.emit("room:error", "Join the lobby before opening its chat."); socket.join(roomId); io.to(roomId).emit("room:presence", { roomId, online: (await io.in(roomId).fetchSockets()).length }); });
